...
"""
Solvebool351
Modification variable SIZE: variable globale dans la classe et ses fonctions
version 300 modifié pour éviter la méthode Pertick pendant la recherche des termes supplémentaires
voir __pt_facteur_unique
Tres rapides mais solution moins simplifiée
"""
"""
...
def decode_bin(tup_terme):
    ...
    return "".join(['-' if msq & (1 << k) else '1' if terme & (1 << k )
                else '0' for k in range(self.SIZE-1, -1, -1)])
...
def input_tables_01(data_in):
    ...
    while len(data_in) != 0:
        ...
        if rep == "t1":
            ...
            terme, erreur = acqui_terme(rep, self.SIZE)
            if erreur:
                terme_0 = set()
                terme_1 = set()
                break
...
class Simply:
    ...
    def __call__(self, tbl_1, tbl_0):
        ...
        def __expense_0_1(term_1, term_0):
            ...
            erreur = False
            if len(term_0) == 0:
                term1 = __expense(term_1, self.SIZE)
                term0 = set(range(0, 1 << self.SIZE)) - term1
            elif len(term_1) == 0:
                term0 = __expense(term_0, self.SIZE)
                term1 = set(range(0, 1 << self.SIZE)) - term0
            else:
                term1 = __expense(term_1, self.SIZE)
                term0 = __expense(term_0, self.SIZE)
        ...
        """
        def __terme_supp(pt_croix, table_croix):
            # compter dans liste priorite le nombre de croix dans chaque colonnes
            # (sauf dans les colonnes pointées par pt_croix)
            #enlever les lignes pointées par les colonnes à croix uniques
            #  table_croix = [i for i in table_0_reduit_croix if i & pt_croix]
            while len(table_croix):
                priorite = []
                for i in range(self.SIZE):
                    pr = 0
                    msq = 1 << i
                    for croix in table_croix:
                        if  croix & msq:
                            pr += 1
                    priorite.append(pr)
                # numero de la colonne qui contient le plus de croix
                num_col = max(range(len(priorite)), key=priorite.__getitem__)
                # pointer la colonne et ajouter au pointeur colonnes croix

                pt_croix |= 1 << num_col

                # enlever les lignes dans table des croix

                table_croix = [i for i in table_croix if not(i & pt_croix)]
        ...
        def __termes_supplementaires(table_1_reduit, table_0):
            ...
            for tab in table_1_reduit:
                if not(tab & flag):
                    ...
                    pt_croix = __pt_facteur_unique(tab, table_0, self.SIZE)
                    ...
                    masque_tr_reduit = __terme_supp(pt_croix, table_0_reduit_croix, self.SIZE)
                    priorite = []
        ...
        table_1, table_0, err = __expense_0_1(self.tbl_1, self.tbl_0, self.SIZE)  # listes en binaires des termes 1 et 0

        if err:
            print("---- Error: terms 0 cover terms 1 ----\n\n")
            quit()

        table_essentiel, table_1_restant = __terme_essentiel(table_1, table_0, self.SIZE)
        if 'v' in self.arg_list:
            print( '\n-- discovered', len(table_essentiel), 'essential term(s) --')
            print('-- remains to be covered', len(table_1_restant), '--\n')

        table_terme_supl = __termes_supplementaires(table_1_restant, table_0, self.SIZE)

        if 'v' in self.arg_list:
            print( '-- discovered', len(table_terme_supl), 'additional terms --')

        tbl_synt, cpt= __tbl_synthese(table_1_restant, table_terme_supl, self.SIZE)
