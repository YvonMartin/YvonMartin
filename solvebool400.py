...
# -*- coding: utf-8 -*-
""" Version qui ne rcherche pas au préalable les termes essentiels
...
"""
   ...
   -the essential reduced minterms pas ici)
...
#             print('-- remains to be covered', len(table_1_restant), '--\n')
        ...
        table_terme_supl = __termes_supplementaires(table_1, table_0, self.nbr)

        if 'v' in self.arg_list:
            print( '-- discovered', len(table_terme_supl), 'additional terms --')

        tbl_synt, cpt= __tbl_synthese(table_1, table_terme_supl, self.nbr)
