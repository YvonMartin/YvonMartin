...
class Simply:
    ...
    def __call__(self, tbl_1, tbl_0):
        ...
        def __terme_supp(pt_croix, table_croix):
            ...
            compteur = 0
#            flag = False
            while len(table_croix) and compteur < ACCEL: #wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww
#                flag = True
                compteur += 1
                priorite = []
...
#             if flag:
#                 masque_tr_reduit.append(pt_croix)

            msq = ((1 << self.SIZE)-1) ^ pt_croix
            s0 = [[]]
            for croix in table_croix:
                dimac = [cpt+1 for cpt in range(self.SIZE) if (croix & msq & (1<<cpt))]
